...
# on MacOS using Homebrew or on Arch Linux using pacman/paru.
...
arch_installation() {
    if ! command -v paru &>/dev/null; then
        echo -e "${Yellow}${DRY_RUN_PREFIX}AUR helper 'paru' could not be found. Attempting to install...${Reset}"
        ...
        echo -e "${Yellow}${DRY_RUN_PREFIX}Cloning paru from AUR to /tmp/paru...${Reset}"
        if [ ! "$DRY_RUN" = true ]; then
            git clone https://aur.archlinux.org/paru.git /tmp/paru
        fi

        echo -e "${Yellow}${DRY_RUN_PREFIX}Building and installing paru from /tmp/paru...${Reset}"
        if [ ! "$DRY_RUN" = true ]; then
            (cd /tmp/paru && makepkg -si --noconfirm)
        fi

        echo -e "${Yellow}${DRY_RUN_PREFIX}Cleaning up /tmp/paru...${Reset}"
        if [ ! "$DRY_RUN" = true ]; then
            rm -rf /tmp/paru
        fi

        if ! command -v paru &>/dev/null && [ ! "$DRY_RUN" = true ]; then
            echo -e "${Red}❌ Failed to install paru. Please install it manually and re-run.${Reset}"
            exit 1
        elif [ "$DRY_RUN" = true ] && ! command -v paru &>/dev/null; then
            echo -e "${Yellow}[DRY RUN] Paru would be installed. Assuming success for further dry run steps.${Reset}"
        fi
    else
        echo -e "${Green}AUR helper 'paru' is already installed.${Reset}"
    fi
    ...
    echo -e "${Blue}${DRY_RUN_PREFIX}Installing AUR packages（paru -S）${Reset}"
    for package in "${aur_packages[@]}"; do
        # In dry run, if paru itself wasn't found, we can't check paru -Q
        can_check_aur=true
        if ! command -v paru &>/dev/null && [ "$DRY_RUN" = true ]; then
            can_check_aur=false
        fi

        if $can_check_aur && ! paru -Q "$package" &>/dev/null; then
            echo -e "${Green}${DRY_RUN_PREFIX}Installing $package...${Reset}"
            if [ ! "$DRY_RUN" = true ]; then
                paru -S --noconfirm --needed "$package"
            fi
        elif ! $can_check_aur; then # paru not installed, in dry run mode
            echo -e "${Green}${DRY_RUN_PREFIX}Installing $package (assuming not installed as paru check skipped)...${Reset}"
        else
            echo -e "${Yellow}$package is already installed, skipping.${Reset}"
        fi
    done
